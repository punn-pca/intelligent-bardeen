    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 pb-5">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
          <CircleDollarSign className="w-7 h-7 text-emerald-600 dark:text-emerald-400" />
      ...
      {/* Tax Info Card */}
      <div className="bg-slate-900 dark:bg-slate-950 text-white p-5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border border-slate-800">
        <div>
      ...
      {taxData && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase">ภาษีขายรวม (Output VAT 7%)</p>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">คำนวณจากเอกสารขาย {taxData.salesDocsCount} บิล</p>
          </div>

          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase">ภาษีซื้อรวม (Input VAT 7%)</p>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">คำนวณจากเอกสารซื้อ {taxData.purchaseDocsCount} บิล</p>
          </div>

          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase">ภาษีมูลค่าเพิ่มนำส่งสุทธิ (Net VAT)</p>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">ภาษีขาย minus ภาษีซื้อ</p>
          </div>
        </div>
      )}
    </div>
