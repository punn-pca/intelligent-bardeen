          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                customers.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-all">
                    <td className="p-4 font-mono font-bold text-blue-600 dark:text-blue-400 text-xs">{c.code}</td>
                    <td className="p-4 font-semibold text-slate-900 dark:text-white text-xs">
                      <p>{c.name}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-normal truncate max-w-xs">{c.address || '-'}</p>
                    </td>
                    <td className="p-4 font-mono text-xs text-slate-700 dark:text-slate-300">{c.taxId || '-'}</td>
                    <td className="p-4 text-xs text-slate-700 dark:text-slate-300 font-medium">{c.contactPerson || '-'}</td>
                    <td className="p-4 text-xs text-slate-600 dark:text-slate-400">
                      <p>{c.phone || '-'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500">{c.email || ''}</p>
                    </td>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-xl p-6 space-y-4 text-slate-900 dark:text-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">รหัสลูกค้า *</label>
                  <input
                    type="text"
                    required
                    value={formData.code}
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">ชื่อลูกค้า / นิติบุคคล *</label>
                <input
                  type="text"
                  required
                  value={formData.name}
