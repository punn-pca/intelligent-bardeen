...
// Compatibility mirror of FIRE KEEPER shared contract.
// Keep this file dependency-free so Next.js can consume the governance boundary safely.
...
export function validateDecisionObject(decision: unknown): ValidatorResult {
  ...
  if (!result.success) return { status: 'REPAIR_REQUIRED', errors: result.error.issues.map(i => `${i.path.join('.')}: ${i.message}`), metadata: { timestamp: new Date().toISOString(), checkedFields: [] } };
  const data = result.data;
  const errors: string[] = [];
  if (data.recommendation && !data.options.some(o => o.id === data.recommendation!.optionId)) errors.push('Recommendation refers to non-existent option');
  if (data.policy_conflicts.some(c => c.severity === 'CRITICAL')) return { status: 'ESCALATE', errors: ['Critical policy conflict detected'], metadata: { timestamp: new Date().toISOString(), checkedFields: ['policy_conflicts'] } };
  if (data.escalation_required) return { status: 'ESCALATE', errors: errors.length ? errors : ['Decision requires human review'], metadata: { timestamp: new Date().toISOString(), checkedFields: ['escalation_required'] } };
  return { status: errors.length ? 'REPAIR_REQUIRED' : 'PASS', errors, metadata: { timestamp: new Date().toISOString(), checkedFields: Object.keys(data) } };
}
