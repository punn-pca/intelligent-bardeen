...
import {
  ...
  Receipt, Landmark, BrainCircuit, X,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const sidebarContent = (
    <div className="w-64 bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col h-full border-r border-slate-800 shrink-0">
        ...
        {navGroups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="space-y-1">
              <p className="px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">{group.groupName}</p>
